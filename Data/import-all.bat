...
REM Resolve Sanity CLI command (prefer global, fallback to NPX)
set "SANITY_CMD=sanity"
where sanity >nul 2>nul
if errorlevel 1 (
    echo [INFO] Global Sanity CLI not found. Falling back to: npx --yes sanity
    set "SANITY_CMD=npx --yes sanity"
)
...
for %%F in (%FILES%) do (
    ...
    if exist "%%F" (
        ...
        call %SANITY_CMD% dataset import "%%F" "%DATASET%" --replace
        if errorlevel 1 (
